...
const CRITICAL_PATTERNS: RegExp[] = [
  /rm\s+(-rf?|--recursive)\s/, />\s*\/dev\/sd/, /mkfs\./, /dd\s+if=/,
  /:()\s*\{\s*:\|:&\s*\};:/, /curl.*\|\s*(ba)?sh/, /wget.*\|\s*(ba)?sh/,
  /sudo\s/, /chmod\s+777/,
];
const HIGH_PATTERNS: RegExp[] = [
  /\brm\b/, /git\s+push/, /git\s+reset\s+--hard/, /git\s+push\s+--force/,
  /\bkill\b/, /npm\s+publish/, /\bcurl\b/, /\bwget\b/,
];
const MEDIUM_PATTERNS: RegExp[] = [
  /git\s+commit/, /git\s+merge/, /git\s+rebase/, /npm\s+install/,
  /bun\s+(add|install)/, /pip\s+install/, /cargo\s+install/, /mv\s/, /cp\s+-r/,
];

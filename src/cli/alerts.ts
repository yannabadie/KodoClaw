...
export function checkAnomalies(m: SessionMetrics): string[] {
	...
	if (m.toolCallsLastMinute >= ANOMALY_THRESHOLDS.tool_calls_per_minute) {
		alerts.push(
			`[KODO] ${m.toolCallsLastMinute} tool calls/min (threshold: ${ANOMALY_THRESHOLDS.tool_calls_per_minute})`,
		);
	}
	...
	if (m.sessionCostUsd >= ANOMALY_THRESHOLDS.cost_per_session_usd) {
		alerts.push(
			`[KODO] Session cost: $${m.sessionCostUsd.toFixed(2)} — exceeds $${ANOMALY_THRESHOLDS.cost_per_session_usd} threshold`,
		);
	}

...
describe("checkSufficiency", () => {
  test("returns sufficient for rich context", () => {
    const ctx = "Authentication uses JWT. Express backend. PostgreSQL database. Session middleware configured.";
...
describe("rewriteQuery", () => {
  ...
  test("returns original if already long", () => {
    const long = "How does the authentication system handle JWT token refresh with Redis session store?";

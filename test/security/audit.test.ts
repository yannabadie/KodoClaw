...
import { AuditLog, type AuditEntry, ANOMALY_THRESHOLDS } from "../../src/security/audit";
...
describe("AuditLog", () => {
  ...
  test("writes entry as JSONL line", async () => {
    await log.write({
      session: "sess_1", mode: "code", autonomy: "trusted",
      action: "shell_exec", command: "ls", risk_level: "low",
      decision: "auto_approved", injection_score: 0, latency_ms: 50, token_cost: 0.001,
    });
  ...
  test("appends multiple entries to same file", async () => {
    await log.write({ session: "s1", mode: "code", autonomy: "trusted", action: "a1", risk_level: "low", decision: "auto_approved", injection_score: 0, latency_ms: 10, token_cost: 0 });
    await log.write({ session: "s1", mode: "code", autonomy: "trusted", action: "a2", risk_level: "low", decision: "auto_approved", injection_score: 0, latency_ms: 10, token_cost: 0 });
    const files = await readdir(dir);
    expect(files.length).toBe(1);
